...
关于`距离`：上游返回的 `distance_to_station` 语义不统一（实测掌上公交给的是车辆到**下一站**的距离，二十多站开外的车也只有两三百米），直接用会让距离提醒对任何一辆车立刻触发。因此这里用**车辆与上车站的经纬度自行计算直线距离**；上游没给车辆坐标或站点坐标时不显示距离，该条提醒也不会触发，不会拿一个错的数字凑数。
